...
public class ST02_QueriesOnST {
    ...
    static int getSum_util(int i, int sti, int stj, int qi, int qj) {
        if (qj <= sti || qi >= stj) { // {{No Overlap }}
            return 0;  // for Min or max Here we have to return Int.Max or Min accoridngly
        } else if (sti >= qi && qj >= stj) { // {{ Complete overlap }}
            return tree[i];
        } else {// {{ Partial overlap }} ab left or right part m bhej denge ans laan ek liyye 
            int mid = (sti + stj) / 2;
