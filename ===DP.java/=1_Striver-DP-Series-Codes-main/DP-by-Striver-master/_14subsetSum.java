...
public class _14subsetSum {
    public static void main(String[] args) {
        ...
        System.out.println("space opti : " + spaceOpti(arr, target));
...
// Recursion --TC -0(2^N) , Sc-0(N)
    private static boolean solveRec(int[] arr, int target, int i) {
        ...
        boolean take = false;
        if (arr[i] <= target)
            take = solveRec(arr, target - arr[i], i - 1);
 ...
 // Time Complexity: O(N*K)  Space Complexity: O(N*K) + O(N)
    ...
    private static boolean solveMemo(int[] arr, int target, int i, int[][] dp) {
        ...
        if (i == 0)
            return arr[i] == target;
        if (dp[i][target] != 0)
            return dp[i][target] == -1 ? false : true;
        boolean take = false;
        if (arr[i] <= target)
            take = solveMemo(arr, target - arr[i], i - 1, dp);
